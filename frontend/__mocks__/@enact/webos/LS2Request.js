...
/*
let filepath = `${mockDirectory}/${parsedUri.method}.json`;

if (!methods.includes(method)){
	filepath = `${mockDirectory}/${parsedUri.method}_${getHash(parsedUri, params)}.json`;
} */
...
class LS2Request {
	send({
		...
		try {
			...
			// eslint-disable-next-line no-console
			console.log({service, method, parameters});
			console.log(`cannot find mock file ${filepath}`);
			onFailure(err);
			onComplete(err);
		}
		return this;
